...
def main():
  ...
  print("Cada secuencia contiene 6 caracteres, un total de 6 secuencias, (ingrese solamente las letras)")
  dna = []
  #bucle de rango 6, para ingresar las se secuencias de adn
  for i in range(6):
      print("Secuencia nro", i + 1, ":")
      while True:
          #paso la cadena a mayus para evitar problemas, elimino espacios, y reemplazo " por si el usuario las coloca
          sequence = input().upper().strip().replace('"', "").replace(" ", "")
